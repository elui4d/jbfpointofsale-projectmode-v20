If (ok=1)
	vStatusCode:=""
	vSwiped:=False:C215
	vTransID:=vCCTransactionID
	//ccTransactionRequestVoid
	ccTransactionRequestVoidinitial
	
	
	//If (vTRANSamount="") | (vTRANSamount="0")
	If (vTRANSError#"")
		MessageBox(""; vTransError+" Please use the online account to void this transaction."; "OKBox"; "red")
		
		
	Else 
		MessageBox("Void Credit Card Transaction"; "Are you sure you want to VOID Transaction ID: "+vCCTransactionID+"?  This transaction occurred on "+vTRANStransactiondate+" and was a "+vTRANScardtype+" in the amount of $"+vTRANSamount+"."; "YesNoBox"; "Red")
		
		
		If (ok=1)
			
			
			Message_Think("Voiding credit card transaction, please wait... ")
			C_TEXT:C284($vString)
			...
			$vString:=$vString+"<TransactionType>"+"void"+"</TransactionType>"+Char:C90(13)
			...
			$vString:=$vString+"<TransactionID>"+vCCTransactionID+"</TransactionID>"+Char:C90(13)
			//$vString:=$vString+"<CCNumber>"+vCCNumber+"</CCNumber>"+Char(13)
			//$vString:=$vString+"<CCExpDate>"+vCCExpiration+"</CCExpDate>"+Char(13)
			//$vString:=$vString+"<Amount>"+vTransaxCCAmount+"</Amount>"+Char(13)
