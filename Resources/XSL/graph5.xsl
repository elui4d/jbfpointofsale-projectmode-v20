...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="area_stroke_width" select="0" />
  <xsl:param name="area_stroke_color" select="'black'" />
  ...
  <xsl:template match="graph">
    ...
    <svg id="ID_svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 {$viewbox_width} {$viewbox_height}" viewport-fill="{$viewport_color}" viewport-fill-opacity="{$viewport_opacity}">
      ...
      <defs >
        ...
        <clipPath id="ID_background_clip">
        ...
        <xsl:for-each select="settings/x_grid">
          <xsl:if test="text() = 'true'" >
            <g id="ID_x_grid" stroke="{$x_grid_color}" stroke-width="{$x_grid_width}">
              <xsl:variable name="NbLines" select="count(../../x_values/v)-1"/>
...
<xsl:comment>draw Y grid lines SHADOW</xsl:comment>
<xsl:for-each select="settings/y_grid">
  <xsl:if test="text() = 'true'" >
...
<xsl:if test="$y_min &lt; 0">
  <g id="ID_y0_axis" stroke="{$axis_y0_color}" stroke-width="{$axis_width}" visibility = "{$axis_visibility}">
    <line x1="{$graph_left_margin}" x2="{$viewbox_width - $graph_right_margin}" y1="{$y0}" y2="{$y0}"/>
        ...
        <!-- set graph clipping (clip against graph background first & area limit then) -->
        <g clip-path="url(#ID_background_clip)" stroke="{$line_color}" stroke-width="{$area_stroke_width}">
          <g clip-path="url(#ID_area_clip)" >
            <!-- draw polyline -->
            <xsl:for-each select="y_values">
              <xsl:comment>
                draw polyline for data type <xsl:value-of select="string(position())"></xsl:value-of>
              </xsl:comment>
              ...
              <polyline id="ID_graph_{position()}" fill="{$fill_color}" fill-opacity="{$area_opacity}" points="{$points}"/>
