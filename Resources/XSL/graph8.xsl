...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- set this parameter to '' to use database svg folder as base url 
                          to a absolute url path to use a different folder as base url    
  -->
  <xsl:param name="image_base_url" select="'_dummy_'" />
  
  <xsl:param name="image_basename" select="'graph8_image'" />
  <xsl:param name="image_ext" select="'svg'" />
  ...
  <xsl:param name="bar_size" select="40"/>
  ...
  <!-- graph command reserved parameters: please do not override -->
  <xsl:param name="graph_base_url" select="''" />
  ...
  <xsl:template match="graph">
    ...
    <!-- image url -->
    <xsl:variable name="url_image_prefix">
      <xsl:choose>
        <xsl:when test="$image_base_url = '_dummy_'" >
          <xsl:value-of select="concat($graph_base_url,$image_basename)" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat($image_base_url,$image_basename)" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="url_image_suffix" select="concat('.',$image_ext)" />
    ...
    <xsl:variable name="bar_opacity" select="1"/>
    ...
    <svg id="ID_svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 {$viewbox_width} {$viewbox_height}" viewport-fill="{$viewport_color}" viewport-fill-opacity="{$viewport_opacity}">
      ...
      <g id="ID_globals" style="fill:{$font_color};stroke:none;font-family:{$font_family};font-size:{$font_size}px;font-weight:{$font_weight};font-style:{$font_style};text-decoration:{$font_decoration};shape-rendering:{$shape_rendering}">
          ...
          <g id="ID_legend" filter="{$legend_rect_filter}">
            ...
            <g id="ID_legend_bullets" stroke="{$legend_bullet_stroke_color}">
              <xsl:for-each select="legend/v">
                <image xlink:href="{$url_image_prefix}{position()}{$url_image_suffix}"
                      preserveAspectRatio="none"
                      x="{$legend_bullet_x}"
                      y="{$legend_bullet_y+$legend_step_y*(position()-1)}"
                      width="{$legend_bullet_size}"
                      height="{$legend_bullet_size}"
                      filter="{$legend_bullet_filter}"
                      id="{$legend_bullet_name}{position()}"
                      />
              </xsl:for-each>
            </g>
        ...
        <xsl:comment>draw X grid lines SHADOW</xsl:comment>
        <xsl:for-each select="settings/x_grid">
          <xsl:if test="text() = 'true'" >
        ...
        <xsl:for-each select="y_values">
          ...
          <g id="ID_graph_{position()}" opacity = "{$bar_opacity}" stroke="1">
              ...
              <xsl:choose>
                <xsl:when test="$y_min &lt; 0">
                  <xsl:choose>
                    <xsl:when test="number(text()) &gt; 0">
                      <image xlink:href="{$url_image_prefix}{$y_values_index}{$url_image_suffix}"
                             preserveAspectRatio="none" 
                             x="{$pos+$bar_width*(($y_values_index)-1)}"
                             y="{$y0 - $bar_height}"
                             width="{$bar_width}"
                             height="{$bar_height}"
                             filter="{$bar_filter}"
                ...
                <xsl:otherwise>
                  ...
                  <image xlink:href="{$url_image_prefix}{$y_values_index}{$url_image_suffix}"
                         preserveAspectRatio="none"
                         x="{$pos+$bar_width*(($y_values_index)-1)}"
                         y="{$viewbox_height - $graph_bottom_margin - $bar_height2}"
                         width="{$bar_width}"
                         height="{$bar_height2}"
                         filter="{$bar_filter}"
                             />
