...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- layout properties -->
  <xsl:param name="base_margin" select="12" />
  <xsl:param name="base_margin_bar" select="$base_margin div 2" />
  <xsl:param name="base_margin_bargroup" select="$base_margin div 2" />
  ...
  <xsl:param name="legend_rect_stroke_color" select="'none'" />
  ...
  <xsl:param name="bar_group_width_min" select="230" />
  <xsl:param name="bar_width_min" select="10" />
  <xsl:param name="bar_width_max" select="80" />
  <xsl:param name="bar_r" select="6"/>
  <xsl:param name="bar_filter" select="'none'"/>
  <xsl:param name="bar_opacity" select="'1'"/>
  <xsl:param name="bar_fill_opacity" select="'1'"/>
  <xsl:param name="bar_stroke" select="'none'"/>
  <xsl:param name="bar_stroke_opacity" select="'1'"/>
  ...
  <xsl:template match="graph">
    ...
    <xsl:variable name="bar_group_width3" select="exsl4D:max($x_label_width+$base_margin*2,$bar_group_width_min)"/>
    <xsl:variable name="bar_group_width2" select="exsl4D:max($bar_group_width3,($bar_width_min+($base_margin_bar*2))*$y_values_count)"/>
    <xsl:variable name="bar_width" select="$bar_group_width2 div $y_values_count"/>
    <xsl:variable name="bar_width2_temp" select="$bar_width - ($base_margin_bar*2)"/>
 
    <xsl:variable name="bar_width2" select="exsl4D:min($bar_width_max,$bar_width2_temp)"/>
    <xsl:variable name="margin_bar" select="($bar_width - $bar_width2) div 2"/>

    <xsl:variable name="bar_group_width" select="($base_margin_bargroup*2)+$bar_group_width2"/>
    <xsl:variable name="bar_group_offset" select="$base_margin_bargroup"/>

    <!-- viewbox properties -->
    <xsl:variable name="viewbox_width" select="($bar_group_width*$x_values_count)+$graph_left_margin+$graph_right_margin" />
    ...
    <svg id="ID_svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
         viewBox="0 0 {$viewbox_width} {$viewbox_height}" viewport-fill="{$viewport_color}" viewport-fill-opacity="{$viewport_opacity}">
      ...
      <g id="ID_globals" style="fill:{$font_color};stroke:none;font-family:{$font_family};font-size:{$font_size}px;font-weight:{$font_weight};font-style:{$font_style};text-decoration:{$font_decoration};shape-rendering:{$shape_rendering}">
              ...
              <xsl:for-each select="../../x_values/v">
                <xsl:choose>
                  <xsl:when test="(position() != 0) and (position() != $NbLines)">
                    <xsl:variable name="x_line_pos" select="$graph_left_margin+($bar_group_width*position())" />
        ...
        <g id="ID_x_values" visibility = "{$x_label_visibility}" style="fill:{$x_label_color};stroke:none;font-family:{$x_label_font_family};font-size:{$x_label_font_size}px;font-weight:{$x_label_font_weight};font-style:{$x_label_font_style};text-decoration:{$x_label_font_decoration};filter:{$x_label_font_filter}">
          <xsl:for-each select="x_values/v">
            <xsl:variable name="pos"
              select="$graph_left_margin+($bar_group_width*(position()-1))+($bar_group_width div 2)"/>
        ...
        <g clip-path="url(#ID_BackgroundClipPath)">
          ...
          <xsl:for-each select="y_values">
            ...
            <g id="ID_graph_shadow_{position()}" fill="{$bar_shadow_color}" fill-opacity = "{$bar_shadow_opacity}">
              <xsl:for-each select="s">
                ...
                <xsl:variable name="pos"
                              select="$graph_left_margin+($bar_group_width*(position()-1))+$bar_group_offset"/>

                <!-- the bar's height -->
                <xsl:variable name="bar_height" select="math:abs(number(text()))*$y_scale"/>
                <xsl:variable name="bar_r_actual" select="exsl4D:min($bar_height div 2,$bar_r)"/>
                ...
                <xsl:choose>
                    ...
                    <xsl:choose>
                      <xsl:when test="number(text()) &gt; 0">
                        <xsl:call-template name="bar_r_top_shadow">
                          <xsl:with-param name="x" select="$pos+($bar_width*(($y_values_index)-1))+$margin_bar+$bar_shadow_shift_x" />
                          <xsl:with-param name="y" select="$y0 - $bar_height+$bar_shadow_shift_y" />
                          <xsl:with-param name="width" select="$bar_width2" />
                          <xsl:with-param name="height" select="$bar_height" />
                          <xsl:with-param name="r" select="$bar_r_actual"/>
                      ...
                      <xsl:otherwise>
                        <xsl:call-template name="bar_r_btm_shadow">
                          <xsl:with-param name="x" select="$pos+$bar_width*(($y_values_index)-1)+$margin_bar+$bar_shadow_shift_x" />
                          <xsl:with-param name="y" select="$y0+$bar_shadow_shift_y" />
                          <xsl:with-param name="width" select="$bar_width2" />
                          <xsl:with-param name="height" select="$bar_height" />
                          <xsl:with-param name="r" select="$bar_r_actual"/>
                  ...
                  <xsl:otherwise>
                    ...
                    <xsl:call-template name="bar_r_top_shadow">
                      <xsl:with-param name="x" select="$pos+$bar_width*(($y_values_index)-1)+$margin_bar+$bar_shadow_shift_x" />
                      <xsl:with-param name="y" select="$viewbox_height - $graph_bottom_margin - $bar_height2+$bar_shadow_shift_y" />
                      <xsl:with-param name="width" select="$bar_width2" />
        ...
        <xsl:for-each select="y_values">
          ...
          <!-- iterate on y numbers -->
          <g id="ID_graph_{position()}" fill="{$fill_color}" opacity = "{$bar_opacity}" stroke="{$bar_stroke}" stroke-opacity = "{$bar_stroke_opacity}" stroke-width = "{$bar_stroke_width}" filter="{$bar_filter}">
            <xsl:for-each select="s">
              <!-- the bar's x position -->
              <xsl:variable name="pos"
                            select="$graph_left_margin+($bar_group_width*(position()-1))+$bar_group_offset"/>
                  ...
                  <xsl:choose>
                    <xsl:when test="number(text()) &gt; 0">
                      <xsl:call-template name="bar_r_top">
                        <xsl:with-param name="x" select="$pos+($bar_width*(($y_values_index)-1))+$margin_bar" />
